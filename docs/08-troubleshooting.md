...
| **Login accettato ma si resta sulla pagina di login** | App servita in **HTTP puro** su un dominio reale: i cookie di auth hanno il flag `Secure` e il browser li scarta (su `localhost` invece funzionano, è un'eccezione del browser) | Impostare `COOKIE_SECURE=false` nel `.env` e ricreare il frontend (`docker compose up -d`). In alternativa servire l'app via HTTPS |
